# -*- coding: utf-8 -*-
# Generated by Django 1.9.6 on 2016-05-29 13:39
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion
import django.utils.timezone
import model_utils.fields


class Migration(migrations.Migration):

    dependencies = [
        ('api', '0051_performancescore'),
    ]

    operations = [
        migrations.CreateModel(
            name='ContestantScore',
            fields=[
                ('created', model_utils.fields.AutoCreatedField(default=django.utils.timezone.now, editable=False, verbose_name='created')),
                ('modified', model_utils.fields.AutoLastModifiedField(default=django.utils.timezone.now, editable=False, verbose_name='modified')),
                ('contestant', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, primary_key=True, serialize=False, to='api.Contestant')),
                ('rank', models.IntegerField(blank=True, editable=False, help_text=b'\n            The final ranking relative to this contest.', null=True)),
                ('mus_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('prs_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('sng_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('total_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('mus_score', models.FloatField(blank=True, editable=False, null=True)),
                ('prs_score', models.FloatField(blank=True, editable=False, null=True)),
                ('sng_score', models.FloatField(blank=True, editable=False, null=True)),
                ('total_score', models.FloatField(blank=True, editable=False, null=True)),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='PerformerScore',
            fields=[
                ('created', model_utils.fields.AutoCreatedField(default=django.utils.timezone.now, editable=False, verbose_name='created')),
                ('modified', model_utils.fields.AutoLastModifiedField(default=django.utils.timezone.now, editable=False, verbose_name='modified')),
                ('performer', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, primary_key=True, serialize=False, to='api.Performer')),
                ('rank', models.IntegerField(blank=True, editable=False, help_text=b'\n            The final ranking relative to this round.', null=True)),
                ('mus_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('prs_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('sng_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('total_points', models.IntegerField(blank=True, editable=False, null=True)),
                ('mus_score', models.FloatField(blank=True, editable=False, null=True)),
                ('prs_score', models.FloatField(blank=True, editable=False, null=True)),
                ('sng_score', models.FloatField(blank=True, editable=False, null=True)),
                ('total_score', models.FloatField(blank=True, editable=False, null=True)),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_mus_points',
            new_name='mus_points',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_mus_score',
            new_name='mus_score',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_prs_points',
            new_name='prs_points',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_prs_score',
            new_name='prs_score',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_sng_points',
            new_name='sng_points',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_sng_score',
            new_name='sng_score',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_total_points',
            new_name='total_points',
        ),
        migrations.RenameField(
            model_name='songscore',
            old_name='d_total_score',
            new_name='total_score',
        ),
    ]
