...
class CleanPanelistManager(Manager):
    def update_or_create_from_raw(self, item):
        # Extract
        pk = item.id
        # Transform
        ...
        district = district_map.get(item.district)

        season_map = {
            'Fall': self.model.SEASON.fall,
            'Spring': self.model.SEASON.spring,
        }
        season = season_map.get(item.season)
        if district == 'BHS':
            season = self.model.SEASON.spring


        session_map = {
            'Chorus': self.model.SESSION.chorus,
            'College': self.model.SESSION.quartet,
            'Quartet': self.model.SESSION.quartet,
            'Quartet College': self.model.SESSION.quartet,
            'Senior Chorus': self.model.SESSION.chorus,
            'Seniors': self.model.SESSION.quartet,
            'VLQ': self.model.SESSION.chorus,
        }
        session = session_map.get(item.session)

        round_map = {
            'Finals': self.model.ROUND.finals,
            'Quarter-Finals': self.model.ROUND.quarters,
            'Semi-Finals': self.model.ROUND.semis,
            'Youth Finals': self.model.ROUND.finals,
        }
        round = round_map.get(item.round)

        category_map = {
            'MUS': self.model.CATEGORY.mus,
            'PER': self.model.CATEGORY.per,
            'PRS': self.model.CATEGORY.per,
            'SNG': self.model.CATEGORY.sng,
        }
        ...
        # person_id = person_map.get(item.judge)
        panelist = None
...
class CleanSongManager(Manager):
    def update_or_create_from_raw(self, item):
        ...
        session_map = {
            'Chorus Finals': self.model.SESSION.chorus,
            'Chorus Semi-Finals': self.model.SESSION.chorus,
            'Mixed Harmony': self.model.SESSION.quartet,
            'Quartet Finals': self.model.SESSION.quartet,
            'Quartet Finals Colle': self.model.SESSION.quartet,
            'Quartet Finals Senio': self.model.SESSION.quartet,
            'Quartet Finals Seniors': self.model.SESSION.quartet,
            'Quartet Finals Youth': self.model.SESSION.quartet,
            'Quartet Quarter-Fina': self.model.SESSION.quartet,
            'Quartet Quarterfinals': self.model.SESSION.quartet,
            'Quartet Semi-Finals': self.model.SESSION.quartet,
            'Quartet Semifinals':self.model.SESSION.quartet,
            'Seniors': self.model.SESSION.chorus,
            'Seniors Chorus': self.model.SESSION.chorus,
            'Very Large Quartet': self.model.SESSION.chorus,
        }
        session = session_map.get(item.session)

        round_map = {
            'Chorus Finals': self.model.ROUND.finals,
            'Chorus Semi-Finals': self.model.ROUND.semis,
            'Mixed Harmony': self.model.ROUND.semis,
            'Quartet Finals': self.model.ROUND.finals,
            'Quartet Finals Colle': self.model.ROUND.finals,
            'Quartet Finals Senio': self.model.ROUND.finals,
            'Quartet Finals Seniors': self.model.ROUND.finals,
            'Quartet Finals Youth': self.model.ROUND.finals,
            'Quartet Quarter-Fina': self.model.ROUND.quarters,
            'Quartet Quarterfinals': self.model.ROUND.quarters,
            'Quartet Semi-Finals': self.model.ROUND.semis,
            'Quartet Semifinals': self.model.ROUND.semis,
            'Seniors': self.model.ROUND.finals,
            'Seniors Chorus': self.model.ROUND.finals,
            'Very Large Quartet': self.model.ROUND.finals,
        }
        round = round_map.get(item.session)
        ...
        defaults = {
            ...
            'session': session,
            'round': round,
            'appearance_num': item.appearance_num,
            'song_num': item.song_num,
            'legacy_group': item.group_name,
            'legacy_chart': item.song_title,
            'scores': item.scores,
            'song': song,
        }
        return self.update_or_create(
...
class RawPanelistManager(Manager):
    ...
    def update_panelist_num(self):
        ...
        for row in rows:
            rawpanelist = self.get(id=row['id'])
            rawpanelist.num = row['panelist_num']
            rawpanelist.save()
